	...
	<div class="wrap">
		<div class="scwatbwsr_content">
			...
			<div class="scwatbwsr_add">
				...
				<input class="scwatbwsr_add_name" placeholder="<?php echo esc_html__("Bereichsname", "scwatbwsr-translate") ?>" type="text">
				<span class="scwatbwsr_add_button"><i class="fa fa-plus" aria-hidden="true"></i> <?php echo esc_html__("Hinzufügen", "scwatbwsr-translate") ?></span>
			</div>
		...
		if (isset($_SESSION["seats" . $proId])) {
			...
			if ($insetArr) {
				$wpdb->query($wpdb->prepare(
					"INSERT INTO $orderTable (`productId`, `orderId`, `seats`, `schedule`)
				VALUES (%d, %s, %s, %s)",
					$proId,
					$order_id,
					implode(",", $insetArr),
					$_SESSION["schedule" . $proId]
				));
...
function wpf_dev_process($fields, $entry, $form_data)
{
	...
	$wpdb->query($wpdb->prepare(
		"INSERT INTO $table_name (`productId`, `orderId`, `seats`, `schedule`, `name`, `address`, `email`, `phone`, `note`, `total`)
